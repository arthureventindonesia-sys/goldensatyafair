...
// bearer token in localStorage (survives closing the tab) and attach it to
// every Better Auth request (and to server functions, via `@/lib/auth/middleware`).
// Empty everywhere except the preview after a popup sign-in, so the cookie path
