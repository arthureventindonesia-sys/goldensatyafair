  ...
  return (
    <div className="grid gap-4 rounded-xl border border-border bg-card p-5 sm:grid-cols-[auto_1fr] sm:items-center">
      <div className="mx-auto w-40 rounded-lg bg-white p-2 sm:mx-0">
        <QrCode value={qrValue} tone="print" label={`QR checkout agent ${code}`} className="h-full w-full" />
      </div>
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Scan QR atau buka tautan untuk checkout. Kode referal terisi otomatis dan terkunci.
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          <Button asChild size="sm">
            <Link to="/checkout" search={{ ref }}>
              Buka checkout
            </Link>
          </Button>
          <Button
